...
void motor_control_update_velocity(uint8_t motor_id, uint32_t current_ms) {
    ...
    if (dt > 0.0f) {
        motors[motor_id].velocity = (delta_count * 2 * 3.14159f) / (1000.0f * dt);  // rad/s
    }
...
int16_t motor_control_pid_update(uint8_t motor_id) {
    ...
    if (motors[motor_id].integral > MAX_INTEGRAL) motors[motor_id].integral = MAX_INTEGRAL;
    if (motors[motor_id].integral < -MAX_INTEGRAL) motors[motor_id].integral = -MAX_INTEGRAL;

    float derivative = (error - motors[motor_id].prev_error) * CONTROL_HZ;
    motors[motor_id].prev_error = error;

    float output = PID_KP * error + PID_KI * motors[motor_id].integral + PID_KD * derivative;
